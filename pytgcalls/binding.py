...
#         atexit.register(cleanup)
    ...
    async def ping(self) -> float:
        ...
        await self._send({"ping_with_response": True, "sid": session})
                        ...
                        try:
                            ...
                            if "ping_with_response" in json_out:
                                ...
                                if session_id in self._waiting_ping:
                                    self._waiting_ping[session_id].set_result(None)
                            if "ping" in json_out:
    ...
    async def _send_response(self, json_data: dict, uid: str):
        if self._ssid:
            await self._send({"data": json_data, "uid": uid, "ssid": self._ssid})

    async def _send_error(self, err_mess: str, uid: str):
        if self._ssid:
            await self._send({"err_mess": err_mess, "uid": uid, "ssid": self._ssid})

    async def send(self, json_data: dict):
        await self._send({"data": json_data, "ssid": self._ssid})
    ...
    async def stop(self):
        ...
        try:
            ...
            py_logger.error(f"Error saat stop Node.js: {e}")

        py_logger.info("Node.js subprocess dihentikan")
